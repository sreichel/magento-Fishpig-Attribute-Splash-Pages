 ...
 */
	...
	$this->run("
		...
		CREATE TABLE IF NOT EXISTS {$this->getTable('attributeSplash/page_store')} (
			`page_id` int(11) unsigned NOT NULL auto_increment,
			`store_id` smallint(5) unsigned NOT NULL default 0,
			PRIMARY KEY (`page_id`, `store_id`),
			KEY `FK_PAGE_ID_SPLASH_PAGE_STORE` (`page_id`),
			CONSTRAINT `FK_PAGE_ID_SPLASH_PAGE_STORE` FOREIGN KEY (`page_id`) REFERENCES `{$this->getTable('attributeSplash/page')}` (`page_id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
--		ALTER TABLE {$this->getTable('attributeSplash/page_store')} ADD UNIQUE (`page_id`,`store_id`);
		...
		ALTER TABLE {$this->getTable('attributeSplash/group_store')} ADD UNIQUE (`group_id`,`store_id`);
		...
		try {
			...
			if ($results = $this->getConnection()->fetchAll($select)) {
				foreach($results as $result) {
					$this->getConnection()->insert($this->getTable('attributeSplash/' . $type . '_store'), $result);
				}
			}
		
			$this->getConnection()->dropColumn($this->getTable('attributeSplash/' . $type), 'store_id');
		}
		catch (Exception $e) {
			Mage::logException($e);
		}
